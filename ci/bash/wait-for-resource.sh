...
if [ "${RESOURCE}" = "apb" ] && [ "${ACTION}" = "completed" ] && [ "${RESOURCE_NAME}" = "${aws_svc_name}" ]; then
  ...
  for r in $(seq 10); do
    ...
    if [ -z "${apb_pod_name}" ]; then
      echo "Waiting for ${aws_svc_name_pretty} APB pod to start..."
      sleep 2
    else
      break
    fi
  ...
  if [ -z "${apb_pod_name}" ]; then
    echo "${aws_svc_name} pod doesn't appear to be starting, aborting..."
    exit 3
  else
    ...
    for r in $(seq ${POLL_ATTEMPTS}); do
    	...
    	echo "Waiting for ${aws_svc_name_pretty} APB to complete... ${r}"
    	sleep ${POLL_SLEEP}
    done
  fi
