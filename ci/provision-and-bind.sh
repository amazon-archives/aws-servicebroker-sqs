...
function verify-ci-run {
  # This section is specific to SQS APB
  export AWS_ACCESS_KEY_ID="${aws_access_key}"
  export AWS_SECRET_ACCESS_KEY="${aws_secret_key}"

  print-with-yellow "Retrieving list of SQS queues in ${aws_region}..."
  sqs_queue_arn=$(aws sqs list-queues --region $aws_region | grep $stack_identifier)

  if [ -z "${sqs_queue_arn}" ]; then
    print-with-red "FAILURE - Couldn't find SQS queue in ${aws_region} with expected ARN."
    VERIFY_CI_ERROR=true
  else
    print-with-green "SUCCESS"
    print-with-green "Queue ARN found: ${sqs_queue_arn}"
  fi
